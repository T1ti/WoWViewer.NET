...
public static class FrameTimingCategoryCatalog
{
    private static readonly IReadOnlyDictionary<string, FrameTimingCategory> Categories =
        new Dictionary<string, FrameTimingCategory>(StringComparer.Ordinal)
        {
            ...
            ["M2 particles and ribbons (CPU)"] = Category("M2 particles and ribbons (CPU)", "CPU time building visible M2 particle and ribbon submissions and generating their effect meshes.", "#B487E8", 7),
            ["M2 command submission (CPU)"] = Category("M2 command submission (CPU)", "CPU time preparing M2 instance data, material state, bindings, and draw commands after animation and culling.", "#1F7169", 8),
            ["Terrain culling (CPU)"] = Category("Terrain culling (CPU)", "CPU visibility and level-of-detail selection work for terrain chunks.", "#B8DF78", 9),
            ["Terrain command submission (CPU)"] = Category("Terrain command submission (CPU)", "CPU time preparing terrain buffers, material state, and draw commands after culling.", "#5F8F2E", 10),
            ["Liquids culling (CPU)"] = Category("Liquids culling (CPU)", "CPU visibility and batch selection work for ADT and visible WMO liquid surfaces.", "#8BD7EA", 11),
            ["Liquids command submission (CPU)"] = Category("Liquids command submission (CPU)", "CPU time preparing and submitting liquid surface draw commands.", "#2D7895", 12),
            ["Scene setup (CPU)"] = Category("Scene setup (CPU)", "CPU scene setup before pass culling: camera matrices, lighting, shader state, and shared render state.", "#B6C0CC", 13),
            ["Debug command submission (CPU)"] = Category("Debug command submission (CPU)", "CPU time submitting optional bounds and selection-debug geometry.", "#778899", 14),
            ["Other scene work (CPU)"] = Category("Other scene work (CPU)", "Remaining SceneRenderTime not covered by named categories; typically sky rendering, renderer bookkeeping, and small uninstrumented gaps.", "#566574", 15),
            ["Other frame work (CPU)"] = Category("Other frame work (CPU)", "CPU time outside scene rendering and streaming, including input, editor update work, and render-loop overhead.", "#5EA1FF", 16),
